...
def load_state():
    ...
    state.setdefault("production_reached_trigger", False)
...
def get_solar_data(access_token):
...
def check_production_drop(production, state):
    ...
    print("Checking solar production alert...")

    # First, wait until production has actually reached
    # the configured high level.
    if production >= PRODUCTION_TRIGGER:

        state["production_reached_trigger"] = True

        # A new high-production period can re-arm the alert.
        state["production_drop_alert_sent"] = False

        print(
            f"Solar production reached trigger: "
            f"{production:.2f} kW"
        )

        return

    # Do not alert if production has not previously
    # reached the trigger level.
    if not state["production_reached_trigger"]:
        return

    # Alert once when production drops below the low threshold.
    if production < PRODUCTION_DROP_BELOW:
        ...
        if not state["production_drop_alert_sent"]:
            ...
            send_notification(
                PRODUCTION_DROP_MESSAGE
                + f" Production is now {production:.2f} kW "
                + f"(previously reached {PRODUCTION_TRIGGER:.1f} kW).",
                WARNING_PRIORITY,
                "warning,sunny"
            )
